...
interface ProductInfoProps {
    product: Pick<
        ProductWithTotalPrice, "basePrice" | "name" | "description" | "discountPercentage" | "totalPrice">
}
export const ProductInfo = ({ product: { name, basePrice, description, discountPercentage, totalPrice } }: ProductInfoProps) => {
    const [quanty, setQuanty] = useState(1)
    const handlerIncrement = () => {
        setQuanty((prevState) => prevState + 1)
    }
    const handlerDecrement = () => {
        setQuanty((prevState) => (prevState === 1 ? prevState : prevState - 1))
    }
    return (
        <div className="flex flex-col px-5">
            ...
            {discountPercentage > 0 && (
                <p className=" opacity-75 line-through text-xs">De R${Number(basePrice).toFixed(2)}</p>
            )}
            <div className="flex items-center gap-3 mt-4">
                <Button size="icon" variant="outline" onClick={handlerDecrement}>
                    <ArrowLeftIcon size={16} />
                </Button>
                <span className="text-lg">{quanty}</span>
